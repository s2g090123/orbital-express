...
const lang = require('../../../services/language'); // grab i18n after we configured it. defaults to en locale and defaults to './locales' relative to node_modules directory to grab language json files: https://github.com/mashpie/i18n-node
...
async function V1ExampleTask(job) {
  const i18n = lang.getLocalI18n(); // grab fresh new copy of i18n instead of global i18n - remember to set the locale based on the user e.g. i18n.setLocale(user.locale);
  ...
  try {
    ...
    // const message = i18n.__('USER[put_new_message_key]', { email: 'bob@example.com' });
